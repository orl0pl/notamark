...
async function getNote(id: string) {
  const resNote = await fetch(
    (SERVER_HOST || "http://localhost:3000") + "/api/note/" + id,
  );
  ...
  if (id == "loading" || note == "loading") {
    ...
    return (
      <FormComponent
        ...
        formHeader={t("note.delete.confirm", { title: note.title })}
        inputs={[
        ...
        submitTitle={t("note.delete.button")}
        submitUrl="/api/note/delete/"
        redirectUrl={`/subject/${router.query.id?.toString()}/lesson/${router.query.lid?.toString()}`}
      />
    );
  }
}
