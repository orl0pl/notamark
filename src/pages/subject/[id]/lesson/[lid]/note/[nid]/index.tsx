...
async function getNotes() {
  const resNotes = await fetch(
    (SERVER_HOST || "http://localhost:3000") + "/api/notes/",
  );
...
async function getUserName(id: string) {
  const resUser = await fetch(
    (SERVER_HOST || "http://localhost:3000") + "/api/user/" + id,
  );
...
export default function Home() {
  ...
  const [notes, setNotes] = useState<WithId<Note>[] | "loading" | null>(
    "loading",
  );
  const [currentNote, setCurrentNote] = useState<
    WithId<Note> | "loading" | null
  >("loading");
  const [userName, setUserName] = useState<string | "loading..." | null>(
    "loading",
  );
  ...
  useEffect(() => {
    if (lesson !== "loading" && lesson !== null) {
      getNotes()
        .then((x) => {
          setCurrentNote(
            x?.find((n) => n._id.toString() === (router.query.nid || "")) ||
              null,
          );
            ...
            <ListDetailSide className="hidden sm:flex">
              ...
              <ListDetailBody>
                {notes === "loading" ? (
                  ...
                  <NotesList
                    selectedId={router.query.nid?.toString() || ""}
                    notes={notes}
                  />
                ) : (
                  <Center>
            ...
            <ListDetailSide>
              {currentNote === "loading" ? (
                ...
                <ListDetailBody className="p-0.5">
                  <span className="title-large">{currentNote.title}</span>
                  <div className="flex label-large tertiary-text gap-1 md:gap-2 items-center mb-1">
                    <span>
                      {t("note.editcount", {
                        count: currentNote.history.length,
                      })}
                    </span>
                    <span>{t("dot")}</span>
                    <span>
                      {t("note.createdatandby", {
                        author: userName,
                        time: moment(currentNote.createdAt * 1000).fromNow(),
                      })}
                    </span>
                  </div>
