...
export default function Page() {
    ...
    if(id=='loading'){
        ...
        return (
            <FormComponent icon={mdiAccountEdit} formHeader={t('user.form')} inputs={[
                {placeholder: t('user.login'), name: "newLogin"},
                {placeholder: t('user.password'), name: "newPassword", type: "password"},
                {name: "token", type: "hidden", value: router.query.token?.toString()},
            ]} submitTitle={t('user.form')} redirectUrl="/" submitUrl="/api/ticket/resolve"/>
        )
    }
}