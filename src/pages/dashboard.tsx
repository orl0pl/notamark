...
function UsersManagment({ users }: { users: WithId<SafeUser>[] }) {
  ...
  const { data } = useSession();
  const router = useRouter();
  const [selectedUser, setSelectedUser] = useState<WithId<SafeUser> | null>(
    null,
  );
  const [sessionUser, setSessionUser] = useState<
    UserAndSession | "loading" | null
  ...
  async function updateUser() {
    ...
    if (
      sessionUser.login === selectedUser.login &&
      sessionUser.accountLevel === 2
    ) {
  ...
  return (
    <ListDetailContainer className="flex-col justify-end sm:flex-row">
          ...
          {users.map((user, i) => {
            return (
              ...
              >
                ...
                <CardDetailsContainer>
                  {t("user.accountlevel")}:
                  {" " + t("user.level." + user.accountLevel.toString())}
                </CardDetailsContainer>
              </CardContainer>
            );
          })}
      ...
      <ListDetailSide>
        <ListDetailTitle>
          {selectedUser ? t("user.edit") : t("user.clicktoedit")}{" "}
        </ListDetailTitle>
          ...
          {selectedUser && (
            <>
              ...
              <Input
                disabled
                type="password"
                value={"niemogepodachaslabotoniefair"}
              />
...
export default function Home({ users }: { users: WithId<SafeUser>[] }) {
  const { t } = useTranslation();
  const router = useRouter();
